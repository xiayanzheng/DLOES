...
class Msg(object):
    CurrentExtension = ("Python.DLOES:当前目标扩展名为")
    ConnDataBaseFail = ("数据库连接失败")
    DefultPathNotFound = ("Python.DLOES:未找到默认路径,请设定默认路径\nAdmin:")
    AddDefultPathSuccess = ("Python.DLOES:默认路径添加成功")
    DelDefultPathFaild = ("Python.DLOES:默认路径添加失败")
    PathDoseNotExists = ("Python.DLOES:路径不存在或无法访问,请重试.")
    PathOption = ("Python.DLOES:当前默认路径为<<%s>>\n      "
                            "按<1>使用回到主菜单\n      "
                            "按<2>自定义路径\n      "
                            "按<3>更新默认路径\n      "
                            "或按任意键使用默认路径继续\nAdmin:"% DLOExtendService.PathList[-1])

    def __init__(self,CurrentExtension):
...
class DLOExtendService(object):
    ...
    def __init__(self, NumberOfDeletedFiles, SizeOfDeletedFiles, Log,TimeList,PathList):
            ...
            if len(DLOExtendService.DatabaseAdapter(self, SQL="SELECT * FROM FILELOCATION", Data=None)) == 0:
                ...
                if os.path.exists(SetDefultPath) != False:
                    if DLOExtendService.DatabaseAdapter(self, SQL="INSERT INTO FILELOCATION(PATH) VALUES(?)",
                        ...
                        print(Msg.AddDefultPathSuccess)
                        DLOExtendService.Get(self,"TargetPath")
                    else:
                        print(Msg.DelDefultPathFaild)
                        DLOExtendService.Flow(self)
                else:
                    print(Msg.PathDoseNotExists)
                    DLOExtendService.Get(self, "TargetPath")
            else:
                DLOExtendService.PathList
                for Data in DLOExtendService.DatabaseAdapter(self, SQL="SELECT * FROM FILELOCATION", Data=None):
                    DLOExtendService.PathList.append(Data[0])
            GetPath = input(Msg.PathOption)
        ...
        if Mode == ("Time"):
            if input("Python.DLOES:按<1>回到主菜单\n      或按任意键设定开始与结束时间\nAdmin:") == ('1'):
                ...
                try:
                    ...
                    if input("Python.DLOES:上一次运行时间为%s\n      是否使用此时间<加上一天%s>\n      作为删除开始时间?(Y/N)\nAdmin:" % (GetLastRunTime,LastRunTime)) in ['y', 'Y']:
    ...
    def Flow(self):
		...
		\n%s
		\n+++++++++++++++++++++++++++++++++++++++
                ''' % (DLOExtendService.NumberOfDeletedFiles, int(DLOExtendService.SizeOfDeletedFiles / 1024 / 1024),
                       DLOExtendService.Logger(self,"LastRunTime",None,None))))
        print("+++++++++++++++++++++++Done++++++++++++++++++++++++")
        os.system('pause')
