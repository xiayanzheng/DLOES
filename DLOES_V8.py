...
class DLOExtendService():
        ...
        if ChoiceForTime == ('1'):DLOExtendService.CommandLineInterface(self)
            ...
            try:
                ...
                if len(DLOExtendService.GetLastRunTime(self))<1:
                    return [StartTime,EndTime,GetStartTime,GetEndTime]
                else:
                    if EndTime < StartTime:
                        ...
                        if StartTime or EndTime < DLOExtendService.GetLastRunTime(self)[-1]:
                            ...
                            if OverWriteExitesTime in ["y","Y"]:
                                ...
                                DLOExtendService.GetTime(self)
                        else:return [StartTime,EndTime,GetStartTime,GetEndTime]
            except:
    ...
    def CommandLineInterface(self):
        ...
        if SelectFunction[0] == 'KLO':
            DLOExtendService.FileFilterKeepLatestVersion(self, TargetPath, StartTime, EndTime,GetExtensionList)
        elif SelectFunction[0] == 'DA':
            DLOExtendService.FileFilterDiscardAll(self, TargetPath, StartTime, EndTime,GetExtensionList)
        print(('''\n+++++++++++++++++++++++++++++++++++++++
		\n共删除%s个文件
		\n已释放%sMB磁盘空间
		\n%s
