...
class DLOExtendService(object):
    ...
    def __init__(self, NumberOfDeletedFiles, SizeOfDeletedFiles, Log,LogFolderSize,TimeList):
        self.NumberOfDeletedFiles = NumberOfDeletedFiles
        self.SizeOfDeletedFiles = SizeOfDeletedFiles
        self.Log = Log
        self.TimeList = TimeList
        self.LogFolderSize = LogFolderSize
    ...
    def Get(self,Mode):
        ...
        if Mode == ("Time"):
            ...
            if DLOExtendService.TimeList[0] > DLOExtendService.TimeList[1]:
                ...
                return [DLOExtendService.TimeList[0], DLOExtendService.TimeList[1]+86390,
                        time.strftime("%Y-%m-%d", time.localtime(DLOExtendService.TimeList[0])),
                        time.strftime("%Y-%m-%d", time.localtime(DLOExtendService.TimeList[1]+86390))]
    ...
    def Flow(self):
        ...
        DLOExtendService.Logger(self,("Maintain"),LogsPath)
        os.system('cls')
        ConfirmInfo = input('''Python.DLOES:请确认设定:
        \n      此操作将%s
        \n      目标文件夹及所有子文件夹:<<<%s>>>>
        \n      开始时间:<<<%s>>>>
        \n      结束时间:<<<%s>>>>
		...
		\n%s
		\n+++++++++++++++++++++++++++++++++++++++
                ''' % (DLOExtendService.NumberOfDeletedFiles,DLOExtendService.Logger(self,"LastRunTime",None))))
        print("+++++++++++++++++++++++Done++++++++++++++++++++++++")
        os.system('pause')
    ...
    def DeleteExpiredFile(self, FilePath):
        try:
            ...
            DLOExtendService.Logger(self,("DeletedFiles"),FilePath)
            try:
               os.remove(FilePath)
    ...
    def Logger(self,Mode,FilePath):
        if Mode == ("LastRunTime"):
            ...
            for Root, Dirs, Files in os.walk(FilePath):
                for File in Files:
                    DLOExtendService.LogFolderSize += os.path.getsize(os.path.join(Root, File))
            if DLOExtendService.LogFolderSize > 10240:
                Selection = input("Python.DLOES:日志文件大小依超过10MB,是否进行日志清除?(Y/N)\nAdmin:")
                if Selection in ["y","Y"]:
                    StartTime = 1459175064.0
                    EndTime = time.time()
                    ExtensionList = ['.txt']
                    DLOExtendService.FileFilter(self, ("KLO"), FilePath, StartTime, EndTime, ExtensionList)
                else:pass
            else:pass
